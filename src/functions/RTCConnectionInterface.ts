...
import { DeviceList, RTCConnectionInterfaceOptions, SocketData } from "./types";
...
export class RTCConnectionInterface {
    ...
    initRTC(): void {
        ...
        this.pc.addEventListener("iceconnectionstatechange", () => {
            ...
            switch (this.pc.iceConnectionState) {
                ...
                case "completed":
                    this.connected = true;
                    if (this.options.onConnected != null)
    ...
    handleChannelData(data: SocketData): void {
        if (typeof data === "object" && "function" in data) {
            const func =
                this.functions[(data as { function: string }).function];
    ...
    negotiate(hash: string, videoEnabled = false): void {
        ...
        this.pc
            ...
            .then((offer: RTCSessionDescription) => {
                ...
                if (this.pc == null)
                    throw new Error(
                        "PeerConnection not initialized on negotiate",
                    );
            ...
            .then(() => {
                ...
                if (offer != null) {
                    offer._sdp = this.sdpFilterCodec(
                        "video",
                        "VP8/90000",
                        offer._sdp,
                    );
    ...
    stop(): void {
        if (this.pc == null) throw new Error("PeerConnection not initialized");
        this.pc.close();
        if (this.options.onConnectionStop != null)
            this.options.onConnectionStop();
    }
}
